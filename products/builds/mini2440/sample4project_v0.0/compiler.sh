...
#   ModifiedTime: 2017-12-11 01:21:38
#
# Revision:
#
...
_for_OS_="for_linux"
_out_="$(pwd)/../../../${_platform_}/${_for_OS_}/v$(date +%Y%m%d%H%M)/${_project_}/${_version_}"
...
if [ -e "${_env4qemu_}" ]; then
    ...
    read -p "JLLim..| Rebuilding Qemu if press [y], or skip:   " _choice_
    echo
fi
